...
uint8_t accelWarningState = 0;
uint8_t lightWarningState=0;
static char* msg = NULL;
static char* msg_temp = "Temp. too high";
static char* msg_accel = "Veer off course";
static char* msg_light = "Obstacle near";
...
void tempWarning(){
	if (tempWarningState==1){
		rgb_setRedBlue(1,0);
		UART_Send(LPC_UART3, (uint8_t *)msg_temp , strlen(msg_temp), BLOCKING);
	}
}

void accelWarning(){
	if (accelWarningState==1){
		rgb_setRedBlue(0,1);
		UART_Send(LPC_UART3, (uint8_t *)msg_accel , strlen(msg_accel), BLOCKING);
	}
}

void lightWarning(){
	if (light_read()>600){
		UART_Send(LPC_UART3, (uint8_t *)msg_light , strlen(msg_light), BLOCKING);
	}
}
